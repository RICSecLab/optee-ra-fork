...
# Yocto directory location - can be overridden via environment variable
# Default: ./yocto (relative to script directory)
# For faster builds on tmpfs: export YOCTO_DIR=/dev/shm/yocto
# Note: /dev/shm may have issues with some packages (e.g., gdk-pixbuf-native)
YOCTO_DIR="${YOCTO_DIR:-$SCRIPT_DIR/yocto}"
mkdir -p "$YOCTO_DIR"

# Create symlink if YOCTO_DIR is not the default location
if [ "$YOCTO_DIR" != "$SCRIPT_DIR/yocto" ]; then
    ln -sfn "$YOCTO_DIR" "$SCRIPT_DIR/yocto"
fi
