...
case "$PLATFORM" in
    qemu)
        ...
        if [ -d "/optee" ]; then
            ...
            # Set TEEC include and lib paths for the host build
            export TEEC_INCLUDE=/optee/optee_client/libteec/include
            export TEEC_LIB=/optee/out-br/target/usr/lib
        fi
        ;;
    imx)
...
[ -f host/optee_remote_attestation ] && cp host/optee_remote_attestation $OUT_DIR/ 2>/dev/null || true
